...
const CONCERNS = [
  { label:'Hyperpigmentation', before:8, after:3, emoji:'🟤' },
  { label:'Sun Damage',        before:7, after:4, emoji:'☀️' },
  { label:'Oiliness',          before:6, after:2, emoji:'💧' },
  { label:'Acne Breakouts',    before:5, after:2, emoji:'🔴' },
]
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} className="w-full">
      ...
      {[0,25,50,75,100].map(v => {
        ...
        return <g key={v}>
          <line x1={PAD} y1={y} x2={PAD+iW} y2={y} stroke={BORDER} strokeWidth="1"/>
          <text x={PAD-8} y={y+4} fill={MUTED} fontSize="10" textAnchor="end">{v}</text>
        </g>
      })}
      {data.map((d,i) => (
        <text key={d.month} x={PAD+(i/(data.length-1))*iW} y={H-8} fill={MUTED} fontSize="11" textAnchor="middle">{d.month}</text>
      ))}
      {metrics.map(m => {
        const active = activeMetric==='all' || m.key===activeMetric
        return <g key={m.key} opacity={active ? 1 : 0.12}>
          ...
          {data.map((d,i) => {
            ...
            return <circle key={i} cx={x} cy={y} r={m.key===activeMetric?4:2.5} fill={m.color} stroke="#1e1a16" strokeWidth="1.5"/>
          })}
        </g>
      })}
    </svg>
    ...
    <div className="min-h-screen pt-24 pb-20 px-6" style={{ background: BG }}>
      <div className="max-w-7xl mx-auto">
        ...
        <motion.div initial={{ opacity:0, y:16 }} animate={{ opacity:1, y:0 }} transition={{ duration:0.3 }}
          ...
          <div>
            <div className="flex items-center gap-3 mb-3">
              ...
              <h1 className="font-display text-5xl font-bold" style={{ color: TEXT }}>Glow Tracker</h1>
            </div>
            <p className="text-base ml-4" style={{ color: DIM }}>6-month progress. Your hyperpigmentation is down 62% since January.</p>
          </div>
            ...
            return (
              <motion.div key={m.key}
                ...
                <div className="flex items-center gap-2 mb-3" style={{ color: m.color }}>
                  {m.icon}
                  <span className="text-xs font-medium" style={{ color: DIM }}>{m.label}</span>
                </div>
                <div className="font-display text-3xl font-bold mb-1" style={{ color: TEXT }}>{val}</div>
                <div className="flex items-center gap-1">
                  <TrendingUp size={11} style={{ color:'#00a877' }}/>
                  <span className="text-[11px]" style={{ color:'#00a877' }}>+{delta} this month</span>
                </div>
                <div className="mt-3 h-1.5 rounded-full overflow-hidden" style={{ background: TRACK }}>
        ...
        <motion.div initial={{ opacity:0, y:16 }} animate={{ opacity:1, y:0 }} transition={{ delay:0.2, duration:0.35 }}
          className="rounded-2xl p-8 mb-6" style={{ background: CARD, border:`1px solid ${BORDER}` }}>
          <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
            <h2 className="font-display text-xl font-bold" style={{ color: TEXT }}>6-Month Progress</h2>
            <div className="flex gap-2 flex-wrap">
              <button onClick={() => setActiveMetric('all')}
                className="text-xs px-3 py-1.5 rounded-lg transition-all font-semibold"
                style={{ background:activeMetric==='all'?'#f53d1c':TRACK, color:activeMetric==='all'?'white':DIM, border:`1px solid ${activeMetric==='all'?'#f53d1c':BORDER}` }}>
                All
              </button>
              {METRICS.map(m => (
                <button key={m.key} onClick={() => setActiveMetric(m.key)}
                  className="text-xs px-3 py-1.5 rounded-lg transition-all flex items-center gap-1.5 font-semibold"
                  style={{ background:activeMetric===m.key?`${m.color}15`:TRACK, color:activeMetric===m.key?m.color:DIM, border:`1px solid ${activeMetric===m.key?`${m.color}40`:BORDER}` }}>
        ...
        <motion.div initial={{ opacity:0, y:16 }} animate={{ opacity:1, y:0 }} transition={{ delay:0.25, duration:0.35 }}
          className="rounded-2xl p-8" style={{ background: CARD, border:`1px solid ${BORDER}` }}>
          <h2 className="font-display text-xl font-bold mb-6" style={{ color: TEXT }}>Skin Concerns — Before vs Now</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {CONCERNS.map((c,i) => (
              <motion.div key={c.label} initial={{ opacity:0, x:-12 }} animate={{ opacity:1, x:0 }} transition={{ delay:0.3+i*0.05 }}>
                <div className="flex items-center justify-between mb-2">
                  <span className="text-sm font-medium flex items-center gap-2" style={{ color: TEXT }}>{c.emoji} {c.label}</span>
                ...
                {[['Before',c.before,'#f53d1c'],['Now',c.after,'#00a877']].map(([lbl,val,col]) => (
                  <div key={lbl} className="flex items-center gap-3 mb-1.5">
                    <span className="text-[10px] w-10 text-right font-medium" style={{ color: MUTED }}>{lbl}</span>
                    ...
                    <span className="text-[10px] w-4 font-medium" style={{ color: DIM }}>{val}</span>
                  </div>
                ))}
              </motion.div>
            ))}
        ...
        {logOpen && (
          <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }}
            ...
            <motion.div initial={{ scale:0.92, y:16 }} animate={{ scale:1, y:0 }}
              ...
              style={{ background: CARD, border:`1px solid rgba(245,240,234,0.12)` }}>
              <h3 className="font-display text-2xl font-bold mb-1" style={{ color: TEXT }}>Log Today's Skin</h3>
              <p className="text-sm mb-6" style={{ color: DIM }}>Rate each metric from 1–10</p>
              {METRICS.map(m => (
                <div key={m.key} className="mb-5">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="font-bold text-sm" style={{ color: TEXT }}>{ratings[m.key]}/10</span>
                  </div>
